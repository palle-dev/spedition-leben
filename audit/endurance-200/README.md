...
## Eingefrorener Code für Vergleichstest (ab 19.09.2026)
Der laufende Prozess behält seine geladenen Module. Für JEDE Wiederaufnahme nach Codeänderungen ausschließlich `audit/endurance-200/baseline-source.tgz` nach `/tmp/frachtfieber-endurance-baseline` entpacken, dort `node_modules` auf `/app/node_modules` verlinken und den Wiederaufnahmebefehl in DIESEM Arbeitsverzeichnis ausführen. Das Archiv enthält src/lib/simulation, bench (einschließlich --resume) und Fixture-Code zum unveränderten Baseline-Stand. Nicht mit dem dann aktuellen /app/src fortsetzen. Ergebnisverzeichnis bleibt /tmp/frachtfieber-endurance. Watch und Zusammenfassung weiter aus /app starten.
