...
const STEPS = [
  ...
  { title: "2. Transport disponieren", body: `Öffne Disposition, wähle den angenommenen Auftrag, einen freien Lkw und einen freien Fahrer. Das Backend prüft Kapazität, Zustand, gemeinsamen Standort, Einsatzgrenze und Geld.` },
...
export default function Tutorial() {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-end md:items-center justify-center p-4">
      <div className="glass border border-white/15 rounded-2xl max-w-lg w-full p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-xs uppercase tracking-wider text-lime">Tutorial · Schritt {Math.min(step + 1, 6)} / 6</div>
            <h3 className="text-lg font-semibold text-foreground mt-1">{STEPS[Math.min(step, 5)].title}</h3>
          </div>
          <button onClick={dismiss} className="text-muted-foreground hover:text-foreground transition"><X className="w-5 h-5" /></button>
        </div>
        <p className="text-sm text-muted-foreground mt-3 leading-relaxed">{STEPS[Math.min(step, 5)].body}</p>
