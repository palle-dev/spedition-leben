...
export default function CompanyStories(){
 ...
 const opportunity=a?.status==="done"&&(state.orders||[]).find(o=>o.customer===a.customer&&o.status==="offered"&&o.acceptDeadlineMin>state.gameTime);
 ...
 {opportunity&&<div className="ff-story-opportunity"><strong>Hier könnt Ihr anknüpfen.</strong><p>{a.customer} bietet aktuell einen Transport von {opportunity.fromCity} nach {opportunity.toCity} an. Prüfe Frist und Kapazität im Auftragsmarkt.</p><Link className="ff-story-link" to="/auftraege">Aktuelles Marktangebot prüfen <ArrowUpRight size={15}/></Link><p className="ff-journey-caption">Reguläres Angebot, keine Reservierung. Es gelten die üblichen Fristen und Kosten.</p></div>}
