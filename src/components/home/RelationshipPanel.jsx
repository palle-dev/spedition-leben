...
import { Heart, Gem, Users, Baby, Cake, Sparkles, Loader2 } from "lucide-react";
...
export default function RelationshipPanel({ state, send, showToast }) {
  ...
  async function action(key, label) {
    setBusy(key);
    try {
      await send(key, {});
  ...
  return (
    <div className="glass-coral border border-coral/20 rounded-2xl p-5 lg:p-6">
      ...
      <div className="space-y-2 pt-2 border-t border-white/10">
        ...
        {status === "married" && !pregnancy && children.length < 4 && !canPlanChild && (
          <p className="text-[10px] text-muted-foreground text-center">
            {relationship < 75 ? "Beziehung muss mindestens 75/100 sein." : ""}
          </p>
        )}
        ...
        {/* Tipp zur Beziehungs-Optimierung */}
        {relationship < 75 && (
          <div className="flex items-start gap-1.5 text-[10px] text-muted-foreground/70 px-1 pt-1">
            ...
            <span>Ein romantischer Abend oder gemeinsame Aktivitäten verbessern die Beziehung.</span>
          </div>
        )}
      </div>
    </div>
