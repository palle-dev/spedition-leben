import AdvisorMandate from "@/components/investment/AdvisorMandate";
...
export default function StaffPhoneDialog({employeeId,onClose}){
 ...
 {data?.contact.role==="investment_advisor"&&!draft&&<div className="space-y-3">{["company","private"].map(id=><details key={id} className="rounded-xl border border-white/10 p-3"><summary className="cursor-pointer text-sm">{id==="company"?"Firmenmandat":"Privatmandat"} im Gespräch festlegen</summary><div className="mt-4"><AdvisorMandate key={id+":"+state.investment.advisor.revision} depotId={id} phone/></div></details>)}</div>}
