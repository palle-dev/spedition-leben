...
export default function PlanningTimeline({ rows, horizonStart, horizonEnd, selectedBlockId, onSelectBlock, showPrivateBar, privateAppointments }) {
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-surface/50 overflow-hidden">
      {/* Zeitstempel */}
      <div className="px-3 py-1.5 border-b border-white/10 bg-white/3 text-[11px] text-muted-foreground flex items-center justify-between">
      ...
      <div className="flex border-b border-white/10">
        <div className="w-32 shrink-0 px-2 py-1.5 text-[11px] font-medium text-muted-foreground border-r border-white/10">
          Ressource
        </div>
          ...
          <div className="flex">
            {Array.from({ length: totalDays }, (_, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`flex-1 px-2 py-1.5 text-[11px] font-medium border-r border-white/5 ${
        ...
        {rows.length === 0 ? (
          ...
          rows.map((resource) => (
            <div key={resource.id} className="flex border-b border-white/5 hover:bg-white/2">
              <div className="w-32 shrink-0 px-2 py-2 border-r border-white/10">
