...
export function processDispatcher(state, emp, m, log) {
  ...
  // seit dem letzten Planungsversuch nicht geändert hat. Wenn der letzte Versuch
  // 0 Touren plante und die Auftrags-/Fahrzeuglage unverändert ist, 2h überspringen.
  ...
  if (emp._lastPlanContext === contextKey && emp._lastPlanPlanned === 0 && m - (emp.lastDecisionMin || 0) < 120) {
    return;
  }
