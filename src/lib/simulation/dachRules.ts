...
export const DACH_TARIFFS={version:DACH_RULE_VERSION,asOf:"2026-09-22",eurPerChf:1.05,
 DE:{regional:17.7,standard:30.3,heavy:34.8,electric:0},
...
export function regulatorySteps(steps){
 ...
 for(const s of steps){
  ...
  route.segments.forEach((leg,i)=>{
   ...
   if(leg.distanceKm>0)result.push({...s,regulatory:true,country:leg.country,distanceKm:leg.distanceKm,durationMin:leg.distanceKm});
  });
 }
 return result;
}
