    ...
    <div className="px-4 sm:px-6 lg:px-12 py-6 lg:py-10 max-w-5xl mx-auto space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{state.vehicles.length} Lkw · Wartung 8 Spielstunden, danach Zustand 100.</p>
        </div>
        <button onClick={buy} disabled={buying || state.company.accountCents < 3000000 || openCompany}
          ...
          {buying ? <span className="w-4 h-4 border-2 border-ink/30 border-t-ink rounded-full animate-spin" /> : <Plus className="w-4 h-4" />} Lkw kaufen (30.000 €)
        </button>
      </div>
        ...
        {state.vehicles.map(v => {
          ...
          return (
            <div key={v.id} className="glass border border-white/10 rounded-xl p-4 hover:border-lime/20 transition">
              ...
              <div className="text-xs text-muted-foreground mt-2">Buchwert {formatEuro(v.bookValueCents)} · 12 t · 28 L/100km</div>
