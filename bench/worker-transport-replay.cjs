...
// --save /private/export.json --output /tmp/worker-transport.json
...
require.extensions['.ts']=load;const js=require.extensions['.js'];require.extensions['.js']=(mod,file)=>file.startsWith(path.join(root,'src'))?load(mod,file):js(mod,file);
Date.now=()=>1800000000000;
if(!isMainThread){
 global.self={postMessage:message=>parentPort.postMessage(message)};
 if(workerData.mode==='new')require(path.join(root,'src/lib/simulationWorker.js'));
 else{
  const {executeCommand}=require(path.join(root,'src/lib/simulationAdapter.js')),{compactHistory}=require(path.join(root,'src/lib/historyArchive.js'));
  self.onmessage=async({data:m})=>{try{const data=await executeCommand(m.state,m.command,m.params||{});if(data.state)data.state=await compactHistory(data.state);self.postMessage({id:m.id,data});}catch(e){self.postMessage({id:m.id,data:{error:e.message}});}};
 }
 ...
 function createWorker(mode){
  const native=new Worker(__filename,{workerData:{mode}});handles.push(native);
 ...
 async function main(){
  ...
  const oldWorker=createWorker('old');let newWorker;const client=createSimulationClient(()=>{newWorker=createWorker('new');return newWorker;});
  const rows=[];let id=0;
  for(const [command,params] of [['setOwnerSalary',{dailyWithdrawalCents:10000}],['advanceTime',{minutes:60,silentPhoneAdvance:true}],['advanceTime',{minutes:1440,silentPhoneAdvance:true}],['advanceTime',{minutes:60,silentPhoneAdvance:true}]]){
   let t=performance.now();const old=await new Promise((resolve,reject)=>{oldWorker.onmessage=e=>resolve(e.data.data);oldWorker.onerror=reject;oldWorker.postMessage({id:++id,state:before,command,params});});const oldMs=performance.now()-t;
   if(old.error)throw Error(old.error);before=stage(old.state);
   ...
   const row={command,minutes:params.minutes||0,coldStart:id===1,oldMs,newMs,oldInputBytes:bytes(oldWorker.sent),newInputBytes:bytes(newWorker.sent),oldOutputBytes:bytes(oldWorker.received),newOutputBytes:bytes(newWorker.received),reusedInput:!!newWorker.sent.reuseCold,reusedRows:newWorker.received.reusedRows,exactStateEquality:true};rows.push(row);console.log(JSON.stringify(row));
  }
