...
async function main(){
 ...
 for(let day=1;day<=days+1;day++){
  ...
  let session,post;if(day===days+1){session=new inspector.Session();session.connect();post=method=>new Promise((resolve,reject)=>session.post(method,(e,r)=>e?reject(e):resolve(r)));await post('Profiler.enable');await post('Profiler.start');}
  ...
  const components=Object.fromEntries(Object.entries(s).map(([k,v])=>[k,Buffer.byteLength(JSON.stringify(v)??'null')]));
  const orderStatuses=s.orders.reduce((a,o)=>(a[o.status]=(a[o.status]||0)+1,a),{});
  const row={components,orderStatuses,projectionBytes:Buffer.byteLength(JSON.stringify(s.accounting.journalProjection||{})),archiveCompressedBytes:s.historyArchive.chunks.reduce((n,c)=>n+(c.storedBytes||0),0),day,warmup:day<=2,profiled:day===days+1,engineMs,compactionMs,before,after:quantities(),delivered:s.stats.totalDeliveries-delivered,activeBytes:Buffer.byteLength(JSON.stringify(s)),archivedBlocks:blocks.size,stateHash:require("node:crypto").createHash("sha256").update(JSON.stringify(s)).digest("hex")};rows.push(row);
  fs.writeFileSync(out+'/days.json',JSON.stringify(rows,null,2));console.log(JSON.stringify(row));
 }
 // Engine-only fixture: archive references remain external; never import as a player save.
 fs.writeFileSync(out+'/engine-fixture.json.gz',require('node:zlib').gzipSync(JSON.stringify(s)));
 assert(rows.every(r=>r.delivered>0));
 const measured=rows.filter(r=>!r.warmup&&!r.profiled).map(r=>r.engineMs).sort((a,b)=>a-b);
 fs.writeFileSync(out+'/summary.json',JSON.stringify({node:process.version,baseCommit:arg('--commit','unspecified'),medianMs:measured.length%2?measured[Math.floor(measured.length/2)]:(measured[measured.length/2-1]+measured[measured.length/2])/2,maxMs:Math.max(...measured),rows,method:'Isolated synthetic 250 trucks / 10 branches, 350 drivers, 60 ordinary dispatchers, capital and daily condition/morale floors. 500 synthetic offers/day plus natural market; real engine simulation and deliveries. Two warmup days, remaining consecutive days measured, one additional sampled day. Per-component JSON sizes measured outside engine time. Compaction and simulated archive staging outside engine time; no browser, actual IndexedDB, cloud, production saves or historical endurance process. Not a long-game proof; fleet size and game age need separate tests.'},null,2));
}
