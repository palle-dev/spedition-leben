...
// node bench/tour-completion-replay.cjs --save /private/export.json --baseline /path/to/old/src/lib/simulation --output /tmp/history-metrics.json
...
async function main(){
 ...
 const {processTours}=require(path.join(root,'src/lib/simulation/tourEngine.ts'));
 const candidates=original.tours.filter(t=>t.status==='active'&&t.createdAt<original.gameTime-30*1440&&
 [...t.deployments,t.returnDeployment].filter(Boolean).every(d=>['completed','skipped'].includes(d.status)));
 const repair=structuredClone(original);repair.tours=structuredClone(candidates);
 const expectedRepair=structuredClone(repair),repairLog=[];
 processTours(repair,repair.gameTime,repairLog);processTours(repair,repair.gameTime,repairLog);
 assert.equal(repair.tours.filter(t=>t.status==='completed').length,candidates.length);
 for(const key of Object.keys(expectedRepair))if(!['tours','historyOutbox','historySequence'].includes(key))assert.deepEqual(repair[key],expectedRepair[key],key);
 const versions=(repair.historyOutbox||[]).filter(r=>r.kind==='tourLifecycleVersions');
 assert.deepEqual(versions.map(r=>r.data),candidates);
 assert.equal(repairLog.filter(r=>r.type==='tour_completed').length,candidates.length);
 ...
 for(let day=1;day<=3;day++){
  ...
  for(const key of Object.keys(before)) {
   if(['historyArchive','historyOutbox','historySequence','tours','accounting'].includes(key))continue;
