...
// --save /private/export.json --baseline /path/to/before --output /tmp/order-output.json
...
if(!isMainThread){
 ...
 async function main(){
  ...
  for(const [command,params] of [['setOwnerSalary',{dailyWithdrawalCents:10000}],['advanceTime',{minutes:60,silentPhoneAdvance:true}],['advanceTime',{minutes:1440,silentPhoneAdvance:true}],['advanceTime',{minutes:60,silentPhoneAdvance:true}]]){
   ...
   const row={command,minutes:params.minutes||0,coldStart:id===1,oldMs,newMs,oldInputBytes:bytes(oldWorker.sent),newInputBytes:bytes(newWorker.sent),oldOutputBytes:bytes(oldWorker.received),newOutputBytes:bytes(newWorker.received),reusedInput:!!newWorker.sent.reuseCold,reusedOrders:!!newWorker.sent.reuseOrders,reusedOrderRows:newWorker.received.reusedOrders || 0,reusedRows:newWorker.received.reusedRows,exactStateEquality:true};rows.push(row);console.log(JSON.stringify(row));
  }
