// Isolated synthetic 250-truck comparison; no production state, browser or cloud writes.
...
async function main(){
 ...
 for(let day=1;day<=6;day++){
  ...
  const row={day,warmup:day<=2,profiled:day===6,engineMs,compactionMs,before,after:quantities(),delivered:s.stats.totalDeliveries-delivered,activeBytes:Buffer.byteLength(JSON.stringify(s)),archivedBlocks:blocks.size,stateHash:require("node:crypto").createHash("sha256").update(JSON.stringify(s)).digest("hex")};rows.push(row);
