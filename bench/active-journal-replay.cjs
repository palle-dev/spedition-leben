...
// node bench/active-journal-replay.cjs --save /private/export.json --baseline /path/to/old/src/lib/simulation --output /tmp/history-metrics.json
...
async function main(){
 ...
 for(let day=1;day<=3;day++){
  ...
  for(const key of Object.keys(before)) {
   if(['historyArchive','historyOutbox','accounting'].includes(key))continue;
