...
async function main(){
 ...
 fullPayloadBytes:bytes(full),repeatPayloadBytes:bytes(delta),bytesAvoided:bytes(full)-bytes(delta),archive:archiveStats(state),
 ...
 limits:"Offline payload measurement on copied save. No cloud request or production change. Cloud still stores/loads a full snapshot; active state and archive manifest still transmitted. No browser or network timing."};
