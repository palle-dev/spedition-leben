...
export function getOnboardingBlocker(state) {
  ...
  if (step === "choose_order") {
    ...
    if (offered.length === 0) {
      return { step, blocked: true, reason: "Keine offenen Angebote verfuegbar. Neue Auftraege erscheinen regelmaessig auf dem Markt.", alternative: "Nutze unten „+1 Std“ oder starte die Live-Simulation, um neue Angebote abzuwarten." };
    }
  }
  if (step === "assign_vehicle") {
    ...
    if (freeVehicles.length === 0) {
      return { step, blocked: true, reason: "Kein freier Lkw verfuegbar. Warte bis eine Tour endet oder kaufe einen weiteren Lkw.", alternative: "Prüfe in der Disposition, wann deine Fahrzeuge wieder verfügbar sind." };
    }
    if (freeDrivers.length === 0) {
      return { step, blocked: true, reason: "Kein freier Fahrer verfuegbar. Warte bis ein Fahrer sich erholt hat oder stelle einen neuen Fahrer ein.", alternative: "Prüfe im Personalbereich Ruhezeiten, Abwesenheiten und Einsatzplanung." };
    }
  }
