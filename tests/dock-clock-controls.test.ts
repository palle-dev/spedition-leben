...
it("Zeitsteuerung bietet Start/Pause und beide Vorläufe mit korrekten Aktionen",async()=>{await render();let b=document.querySelector('button[aria-label="Live-Simulation starten"]') as HTMLButtonElement;const controls=Array.from(b.parentElement.querySelectorAll("button")).map(el=>el.getAttribute("aria-label"));expect(controls).toEqual(expect.arrayContaining(["Live-Simulation starten","1 Stunde weiter","1 Tag weiter"]));await act(async()=>b.click());expect(fixture.game.enableAutomation).toHaveBeenCalledTimes(1);fixture.game.automationEnabled=true;await render();await act(async()=>(document.querySelector('button[aria-label="Live-Simulation pausieren"]') as HTMLButtonElement).click());expect(fixture.game.pauseAutomation).toHaveBeenCalledTimes(1);await act(async()=>(document.querySelector('button[aria-label="1 Tag weiter"]') as HTMLButtonElement).click());expect(fixture.game.startBackgroundAdvance).toHaveBeenCalledWith(1440);});
