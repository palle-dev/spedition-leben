...
describe("Büroklänge",()=>{
 ...
 it("senkt Hintergrund bei Telefonaten und stoppt auf Wunsch",async()=>{const a=await import("@/lib/officeAudio");await a.startOfficeAudio();a.setOfficeVolume(.8);a.setOfficeDucked(true);expect(media.volume).toBe(.2);a.setOfficeDucked(false);expect(media.volume).toBe(.8);a.stopOfficeAudio(true);expect(media.pause).toHaveBeenCalled();expect(a.wantsOfficeAudio()).toBe(false);});
