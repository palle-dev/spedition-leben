...
describe("Geführte Telefongespräche",()=>{
 ...
 it("Kundeninfo ist eine echte Entscheidung mit anschließender Maßnahme",()=>{const s=initial(),d=delay(s);let opts=getPhoneProposals(s,{id:d.id,type:"disruption"});const info=opts.find(o=>o.id==="inform_customer");expect(info).toBeTruthy();applyCommand(s,info.command,info.params);expect(d.status).toBe("decision_open");opts=getPhoneProposals(s,{id:d.id,type:"disruption"});expect(opts.some(o=>o.id==="inform_customer")).toBe(false);const act=opts.find(o=>o.id==="accept_delay");applyCommand(s,act.command,act.params);expect(d.status).toBe("completed");});
 ...
 it("allgemeine Kundeninformation bleibt gespeichert und ist idempotent",()=>{const s=initial(),o=s.orders[0];o.status="unterwegs";const deadline=o.deliveryDeadlineMin;const p=getPhoneProposals(s,{type:"delivery_risk",orderId:o.id})[0];applyCommand(s,p.command,p.params);applyCommand(s,p.command,p.params);expect(s.mail.messages.filter(m=>m.dedupKey==="phone_customer:"+o.id)).toHaveLength(1);expect(o.deliveryDeadlineMin).toBe(deadline);expect(JSON.parse(JSON.stringify(s)).orders.find(x=>x.id===o.id).phoneCustomerInformed).toBe(true);expect(getPhoneProposals(s,{type:"delivery_risk",orderId:o.id})).toHaveLength(0);});
