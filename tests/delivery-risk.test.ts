...
describe("Liefergefährdung",()=>{
 ...
 it("laufende Reparatur bleibt als Liefergefährdung sichtbar",()=>{const s=initial();const o=order(s);s.disruptions.items=[{id:"d",type:"technical_defect",status:"measure_running",orderIds:[o.id],cause:"Reparatur dauert an"}];expect(getDeliveryRisks(s)[0]).toMatchObject({disruptionId:"d"});expect(getCommunicationQueue(s).calls).toHaveLength(0);});
 it("offene Defektentscheidung und Auftrag erzeugen keinen doppelten Anruf",()=>{const s=initial();const o=order(s);s.disruptions.items=[{id:"d",type:"technical_defect",status:"decision_open",orderIds:[o.id]}];expect(getCommunicationQueue(s).calls).toHaveLength(0);expect(getCommunicationQueue(s).emails.filter(e=>e.id==="d")).toHaveLength(1);});
